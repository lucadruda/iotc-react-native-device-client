...
const initialValues: Partial<IoTCCredentials> & {
...
class Logger implements IIoTCLogger {
  ...
  setLogLevel(_logLevel: string | IOTC_LOGGING): void {
    return; // no-op
  }
  log(message: string, _tag?: string): void | Promise<void> {
    this.append(message);
  }
  debug(_message: string, _tag?: string): void | Promise<void> {
    return;
  }
}
...
export default function App() {
  ...
  useEffect(() => {
    if (iotc && iotc.isConnected()) {
      iotc.on(IOTC_EVENTS.Properties, prop =>
        onProperties(prop, s => setLogs(c => `${c}\n${s}`)),
      );
      iotc.on(IOTC_EVENTS.Commands, cmd =>
        onCommands(cmd, s => setLogs(c => `${c}\n${s}`)),
      );
    }
  }, [iotc]);
...
function ConnectForm(props: {
  setIotc: Setter<IIoTCClient | null>;
  append: Setter<string>;
}) {
      ...
      {({handleChange, handleBlur, handleSubmit, values, setFieldValue}) => (
        <KeyboardAvoidingView
          ...
          {(Object.keys(values) as (keyof typeof initialValues)[]).map(k => {
              ...
              return (
                <View
                  ...
                  <CheckBox
                    disabled={false}
                    value={values[k]}
                    onValueChange={v => setFieldValue(k, v)}
                  />
                </View>
              );
            }
            ...
            if (
              (values.newDevice && k === 'deviceKey') ||
              (!values.newDevice && k === 'modelId') ||
              (!values.newDevice && k === 'groupKey')
            ) {
              return null;
            }
            return (
              <TextInput
                ...
                onBlur={handleBlur(k)}
                key={k}
                placeholder={k.replace(
                  /^([a-z])([a-z]+)([A-Z])([\S]+)$/,
                  (a, b, c, d, e) => `${b.toUpperCase()}${c} ${d}${e}`,
                )}
          ...
          <Modal visible={submitting} transparent={true} animationType="slide">
            <View
              style={{
                flex: 1,
  ...
  return (
    <>
      ...
      <View style={{flexDirection: 'row', paddingHorizontal: 20}}>
        <TextInput
          style={{flex: 1, height: 30, borderWidth: 0.5, borderColor: 'gray'}}
          value={input}
        ...
        <Button
          ...
          onPress={async () => {
            ...
            payloadStr = payloadStr.replace(/”/g, '"').replace(/--/g, '—');
